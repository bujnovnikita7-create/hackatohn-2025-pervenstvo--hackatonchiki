...
from ui.dialogs import AddSecretDialog
...
class SecretWallet:
    ...
    def ask_password(self, title, prompt):
        #Кастомный диалог для ввода пароля
        from ui.dialogs import PasswordDialog
        return PasswordDialog(self.root, title, prompt, self.current_theme).show()

    def ask_password_for_secret(self, secret_name, action="просмотра"):
        #Диалог для ввода пароля при работе с секретами
        from ui.dialogs import SecretPasswordDialog
    ...
    def setup_ui(self):
        ...
        tk.Label(search_frame, text="Поиск:", bg=self.current_theme["bg"], fg=self.current_theme["fg"]).pack(
        ...
        tk.Label(list_frame, text="Сохраненные секреты:", bg=self.current_theme["bg"],
                 fg=self.current_theme["fg"]).pack(anchor=tk.W)
        ...
        self.secrets_list.pack(fill=tk.BOTH, expand=True, pady=(5, 0))
